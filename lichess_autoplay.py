...
def play_game(board, our_color):
    ...
    while not check_exists_by_class("follow-up"):
        ...
        if our_turn:
            ...
            if manual_move:
                move = driver.find_element(By.XPATH,
                                           "/html/body/div[2]/main/div[1]/rm6/l4x/kwdb[" + str(move_number) + "]").text
                ...
                if config["general"]["MoveType"] == "key" and keyboard.is_pressed(config["general"]["MoveKey"]) or \
                        config["general"]["MoveType"] == "auto":
